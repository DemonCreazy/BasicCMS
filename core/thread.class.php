...
class Thread {
    ...
    public function setRunnable( $_runnable ) {
        if( self::runnableOk( $_runnable ) ) $this->runnable = $_runnable;
        else throw new Exception( $this->getError( Thread::FUNCTION_NOT_CALLABLE ), Thread::FUNCTION_NOT_CALLABLE );
    }
    ...
    public function start() {
        $pid = @ pcntl_fork();
        if( $pid == -1 ) throw new Exception( $this->getError( Thread::COULD_NOT_FORK ), Thread::COULD_NOT_FORK );
        if( $pid )
            // parent 
            $this->pid = $pid;
        else
    ...
    public function stop( $_signal = SIGKILL, $_wait = false ) {
        if( $this->isAlive() )
            ...
            if( $_wait ) pcntl_waitpid( $this->pid, $status = 0 );
    }
    ...
    public function getError( $_code ) {
		return ( isset( $this->errors[$_code] ) ) ? $this->errors[$_code] :
		'&iexcl;No existe este c&oacute;digo de error: '.$_code.'! &iexcl;&iexcl;No invente errores!!';
    }
